...
from panda3d.core import (
    DirectionalLight, AmbientLight, Vec3, Vec4, Material, 
    CullFaceAttrib, Texture, TextureStage
)
...
class SceneManager:
    ...
    def _setup_model(self, data, mat=None, is_skybox=False):
        ...
        model.setScale(*scale) if isinstance(scale, (list, tuple)) else model.setScale(float(scale))
        
        if "pos" in data:
            model.setPosHprScale(*data["pos"], *data.get("hpr", (0, 0, 0)), *model.getScale())
        if data.get("collide"):
    ...
    def load_map(self, map_dir="Maps/Test", json_file="testmap.json"):
        ...
        if self.props:
            Sequence(
                LerpPosInterval(self.props[0], 2.0, Vec3(0, 15, 13), startPos=Vec3(0, 15, 7)),
                LerpPosInterval(self.props[0], 2.0, Vec3(0, 15, 7), startPos=Vec3(0, 15, 13))
            ).loop()

        self.setup_lighting(data.get("directional_light", {}), data.get("ambient_light", {}))
    ...
    def setup_lighting(self, light_data, ambient_data={}):
        ...
        # Micro-bias tuning to prevent self-shadowing acne across flat faces
